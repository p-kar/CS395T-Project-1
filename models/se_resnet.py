...
def senet50(pretrained=True, target_type='regression', **kwargs):
    """Constructs a ResNet-50 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(SEBasicBlock, [3, 4, 6, 3], mode=target_type, **kwargs)
    model.avgpool = nn.AdaptiveAvgPool2d(1)

    if pretrained:
        pre_model = ResNet(SEBasicBlock, [3, 4, 6, 3], **kwargs)
        pre_model.avgpool = nn.AdaptiveAvgPool2d(1)
        state_dict = torch.load('./models/senet50.pth', map_location='cpu')['weight']
